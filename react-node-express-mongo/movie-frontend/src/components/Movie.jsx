...
function Movie() {
    ...
    const searchMovieByBoth = async ()=>{
        const genres = selectedGenres.join("|")
        const languages = selectedLanguage.join("|")
        let res = await fetch(`http://localhost:7000/movies/search?genres=${genres}&languages=${languages}`);
        let movies = await res.json();
        setMovies(movies)
    }
    ...
    const gotoMovieDetails = async (e,title,id)=>{
        console.log(id,title)
        let res = await fetch(`http://localhost:7000/movies/${id}/${title}`);
                        ...
                        {movies.map((movie) => {
                            return (
                                <div className="col-md-3" style={{ minHeight: '400px' }}>
                                    <div className="card" style={{ height: '100%' }} onClick={(e)=>gotoMovieDetails(e,movie.title,movie._id)}>
